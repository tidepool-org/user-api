...
define(['lodash', 'moment', 'mongojs', 'crypto-js'], function(_, moment, mongojs, crypto) {
  ...
  //      user's password
  //  _wipeTheEntireDatabase -- if this is truthy, it will cause the db to be wiped at startup; this 
  //      exists for testing and should not be used on a production database! 
  ...
  var init = function(config, done) {
    ...
    if (_status) {
      ...
      log.warn(msg);
      done(null, { message: msg });
      return;
    }

    cfg = _.clone(config);
    db = mongojs(cfg.mongoConnectionString, ['users'], function(err, result) {
  ...
  var getUser = function(userdata, done) {
    ...
    db.users.find(userquery, function(err, items) {
      if (err) {
          ...
          message: 'Error from mongodb.', error: err });
        log.error('Error with mongo!', err);
        return;
      }
