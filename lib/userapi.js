...
// KJQ -- delete crypto if we don't need it
  ...
  // validates app token and key against the database -- returns the authorization period for this key
  var checkAppToken = function(apptoken, apikey) {
  ...
  var createAuth = function(apptoken, apikey) {
    var duration = checkAppToken(apptoken, apikey);
  ...
  var auth = function(req, res, next) {
    log.info('auth %s %s %s', '(parameters masked)', req.url, req.method);
    // this could be header fields or post parameters
    var apptoken = req.header('X-Tidepool-Application-ID');
    var apikey = req.header('X-Tidepool-API-Token');
    console.log('HERE!');
    if (!apptoken || !apikey)
    {
      res.send(400, 'Both X-Tidepool-Application-ID and X-Tidepool-API-Token headers are required.');
    }
    else
    {
      var token = createAuth(req.params.apptoken, req.params.apikey);
      if (token) {
        res.send({token:token});
      } else {
        res.send(400, {response:'Authentication failed.'});
      }
    }
    return next();
  };
  ...
  var v01api = [
    ...
    { verbs: ['get', 'post'], path: '/user/auth', func: echo }
  ];
