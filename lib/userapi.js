...
module.exports = (function() {
  ...
  secret = 'this is a secret that should be replaced in configuration';
  ...
  var getSessionToken = function(userid, isServer) {
    ...
    if (isServer) { duration = 60 * 60 * 24; } // server tokens last a day
  ...
  var serverLogin = function(req, res, next) {
    log.info('serverLogin', req.method);
  ...
  var checkSession = function(req, res, next) {
    ...
    verifyToken(req, function(err, result) {
      if (result.statuscode == 200) {
        var response = { userid: result.data.usr };
