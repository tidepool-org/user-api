...
module.exports = function (envConfig, userService) {
  ...
  Password recovery:
    Here's the design thinking:
      ...
      * [ ] Hitting the password recovery endpoint should:
        ...
        * [x] The PW recovery link also can't require a login
      ...
      * [x] There must be some sort of record on the user account that a password recovery was attempted and when.
    ...
    contains two subfields: id is the random recovery key that was generated by the first part of the recovery
    process, and expires is the last date/time for which the id is still valid.
  ...
  */
          ...
          if (result.detail.length !== 1) {
            ...
            userService.generateUniqueHash([req.params.user, envConfig.saltDeploy, moment().valueOf()],
              8, function (linkid) {
                ...
                // save the linkid in the user data
                var updates = {pwrecovery: {id: linkid, expires: moment().add('days', 3).valueOf }};
                userService.updateUser(result.detail.userid, updates, function (err, updated) {
                  if (err != null) {
                    res.send(500, 'Recovery Update No Good');
                    return next();
                  }
                  // and finally, send the recovery email
  ...
  // This is the function that gets called when the user clicks the recovery link
  var passwordRecovery2 = function (req, res, next) {
    userService.getUser({
      userid: req.params.userid
    }, function (err, result) {
      if (result.statuscode == 200) {
        // check that we have the right code and that it's not expired
        if (result.detail.pwrecovery &&
          result.detail.pwrecovery.id == req.params.scramblecode &&
          parseInt(result.detail.pwrecovery.expires, 10) > parseInt(moment().valueOf(), 10)) {
          res.send(200, 'Password has been recovered. (Not really - this needs to do something)');
        } else {
          // not a valid pw recovery link
          res.send(404, 'Password recovery link not valid.');
        }
      } else {
        res.send(404, 'UserID not valid.');
      }
      return next();
    });
  };
  ...
  var v01api = [
    ...
    { path: '/passwordrecovery2/:userid/:scramblecode', verb: 'get', func: passwordRecovery2 },
