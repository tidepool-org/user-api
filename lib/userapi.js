      ...
      userService.generateUniqueHash(strings, 24, function (result2) {
        userService.getUser({userid: req.params.userid }, function (err, item) {
          ...
          case 'GET':
            ...
            return next();
          case 'POST':
            console.log('userinfo.private', userinfo.private, req.params.key);
            if (userinfo.private && userinfo.private[req.params.key]) {
              res.send(422, 'key already in use');
              return next();
            }
            userService.updateUser(userinfo.userid, updates, function (err, updated) {
              ...
              console.log('Status is!', res.statusCode);
                res.send(status, updated.detail.private[req.params.key]);
                return next();
              });
            break;
          case 'PUT':
            ...
            userService.updateUser(userinfo.userid, updates, function (err, updated) {
                res.send(updated.statuscode, updated.detail.private[req.params.key]);
                return next();
              });
            break;
          case 'DELETE':
            ...
            break;
          default:
            res.send(500, 'manageIdHashPair default case');
            break;
          }
          return next();
        });
