...
module.exports = function (envConfig, userService) {
  ...
  var metrics = require('./metrics.js').createMetrics(envConfig, log);
  ...
  var track = function(eventname, extras) {
    return function (req, res, next) {
      var servertoken = req.headers['x-tidepool-session-token'];
      if (servertoken) {
        log.info('metrics call with eventname %s', eventname);
        metrics.post(eventname, extras, servertoken, next);
        log.info('metrics call ends');
      } else {
        log.info('track called in userapi with no servertoken');
        next();
      }
    };
  };
  ...
  var v01api = [
    { path: '/status', verb: 'get', func: [track('status'), status ]},
    { path: '/user', verb: 'post', func: [track('user', {verb: 'post'}), createUser ]},
    { path: '/user', verb: 'get', func: [track('user', {verb: 'get'}), getUserInfo ]},
    { path: '/user', verb: 'del', func: [track('user', {verb: 'del'}), deleteUser ]},
    { path: '/user', verb: 'put', func: [track('user', {verb: 'put'}), updateUser ]},
    ...
    { path: '/serverlogin', verb: 'post', func: [track('serverlogin'), serverLogin] },
