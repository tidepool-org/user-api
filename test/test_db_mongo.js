...
describe('db_mongo', function() {
    ...
    var user1 = {userid: 'Testy', email: 'mctesty@mctester.com', pwhash: 'test2', extras: {}};
    var user2 = {userid: 'McTesty', email: 'mctesty@tester.com', pwhash: 'test', extras: {junk: 'bar'}};

    var checkResult = function(user, ref) {
        expect(user).to.have.property('userid');
        expect(user.userid).to.equal(ref.userid);
        expect(user).to.not.have.property('pwhash');
        expect(user).to.have.property('emails');
        expect(user.emails.length).to.equal(1);
        expect(user.emails[0]).to.equal(ref.email);
        expect(user).to.have.property('extras');
        expect(user.extras).to.eql(ref.extras);
    };
    ...
    it('should find a user by username', function(done) {
        dbmongo.getUser({user: user1.userid}, function(result) {
            expect(result.success).to.be.true;
            checkResult(result.user, user1);
            done();
        });
    ...
    it('should find a user by email', function(done) {
        dbmongo.getUser({user: user2.email}, function(result) {
            expect(result.success).to.be.true;
    ...
    it('should fail to delete a user with an invalid password', function(done) {
        dbmongo.deleteUser({userid: user1.userid, pwhash: 'zzz'}, function(result) {
            expect(result.success).to.be.false;
            expect(result.message).to.equal('User/password combination not found.');
            done();
        });
    ...
    it('should delete a user with a valid password', function(done) {
        dbmongo.deleteUser({userid: user1.userid, pwhash: user1.pwhash}, function(result) {
            expect(result.success).to.be.true;
